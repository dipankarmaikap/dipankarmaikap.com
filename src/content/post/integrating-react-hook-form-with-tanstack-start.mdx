...
ogFilename: src/lib/getFlashData.ts
ogLanguage: typescript
ogCode: |-
  export const flash = createServerFn()
  .handler(async () =>
    JSON.parse(getCookie("flashData")),
  );
---
...
Create a schema with Zod for type safety and reusability.

```tsx src/app/home/schema.ts
import { z } from "zod";
...
Let’s extract validation and response logic into reusable utility.

### Validate Form Data

```tsx src/lib/form-helper.ts
